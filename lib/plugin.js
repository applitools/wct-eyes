...
module.exports = (wct, pluginOptions) => {
  ...
  wct.on('eyes:open', async (def, data, stats, browser) => {
    ...
    try {
      ...
      await browser.execute(`window.top.eyesEmitter.emit('eyes:openDone', ${JSON.stringify({ sessionId })})`);
    } catch (e) {
  ...
  wct.on('eyes:checkWindow', async (def, data, stats, browser) => {
    ...
    try {
      ...
      await browser.execute(`window.top.eyesEmitter.emit('eyes:checkWindowDone', ${JSON.stringify(matchResult)})`);
    } catch (e) {
  ...
  wct.on('eyes:close', async (def, data, stats, browser) => {
    ...
    try {
      ...
      await browser.execute(`window.top.eyesEmitter.emit('eyes:closeDone', ${JSON.stringify(results)})`);
  ...
  wct.on('eyes:abortIfNotClosed', async (def, data, stats, browser) => {
    ...
    try {
      if (eyes) { // sometimes during debugging when multiple browsers open, it is possible that session can't be found
        ...
        await browser.execute(`window.top.eyesEmitter.emit('eyes:abortIfNotClosedDone', ${JSON.stringify(testResults)})`);
      } else {
        wct.emit('log:debug', 'WctEyes: abortIfNotClosed skipped, the session is not open');
        await browser.execute('window.top.eyesEmitter.emit(\'eyes:abortIfNotClosedDone\')');
      }
    } catch (e) {
