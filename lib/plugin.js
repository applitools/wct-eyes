...
module.exports = (wct, pluginOptions) => {
  ...
  wct.on('sub-suite-start', async (def, data, stats, browser) => {
    if (pluginOptions.subsuitesWidth) {
      wct.emit('log:debug', 'WctEyes: subsuitesWidth option given, updating document styles...');

      let requiredWidth = pluginOptions.subsuitesWidth.toString();
      if (!requiredWidth.endsWith('px')) requiredWidth += 'px';

      await browser.execute(`document.getElementById('subsuites').style.width = "${requiredWidth}";` +
                      `document.getElementById('mocha').style.left = "${requiredWidth}";`);

      wct.emit('log:debug', 'WctEyes: subsuitesWidth document styles update done');
    }
  });
  ...
  wct.on('eyes:open', async (def, data, stats, browser) => {
    ...
    try {
      await eyes.open(driver, data.appName, data.testName);
      ...
      await browser.execute(`eyesEmitter.emit('eyes:openDone', ${JSON.stringify({ sessionId })})`);
    } catch (e) {
  ...
  wct.on('eyes:checkWindow', async (def, data, stats, browser) => {
    ...
    try {
      ...
      await browser.execute(`eyesEmitter.emit('eyes:checkWindowDone', ${JSON.stringify(matchResult)})`);
    } catch (e) {
  ...
  wct.on('eyes:close', async (def, data, stats, browser) => {
    ...
    try {
      ...
      await browser.execute(`eyesEmitter.emit('eyes:closeDone', ${JSON.stringify(results)})`);
  ...
  wct.on('eyes:abortIfNotClosed', async (def, data, stats, browser) => {
    ...
    try {
      if (eyes) { // sometimes during debugging when multiple browsers open, it is possible that session can't be found
        ...
        await browser.execute(`eyesEmitter.emit('eyes:abortIfNotClosedDone', ${JSON.stringify(testResults)})`);
      } else {
        wct.emit('log:debug', 'WctEyes: abortIfNotClosed skipped, the session is not open');
        await browser.execute('eyesEmitter.emit(\'eyes:abortIfNotClosedDone\')');
      }
    } catch (e) {
