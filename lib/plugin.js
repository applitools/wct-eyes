...
const { Target, BatchInfo, TestResultsFormatter, ClassicRunner, VisualGridRunner } = require('@applitools/eyes-selenium');
...
module.exports = (wct, pluginOptions) => {
  ...
  wct.hook('configure', (done) => {
    ...
    if (pluginOptions.runner && pluginOptions.runner.toLowerCase() === 'visualgrid') {
      eyesRunner = new VisualGridRunner();
    } else {
  ...
  wct.on('eyes:open', async (def, data, stats, browser) => {
    ...
    try {
      ...
      await eyes.navigateToFrameId(data.frameId);
