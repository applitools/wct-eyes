...
module.exports = (wct, pluginOptions) => {
  ...
  wct.on('eyes:close', (def, data, stats, browser) => {
    ...
    eyes.close(data.throwEx).then((testResults) => {
      ...
      let testResults;
      if (typeof err.getTestResults !== 'undefined') {
        testResults = err.getTestResults();
        eyesResults[data.sessionId] = testResults;
      }

      const results = { passed: false, message: err.message, testResults };
      browser.execute(`eyesEmitter.emit('eyes:closeDone', ${JSON.stringify(results)})`);
  ...
  wct.on('eyes:abortIfNotClosed', (def, data, stats, browser) => {
    ...
    if (eyes) { // sometimes during debugging when multiple browsers open, it is possible that session can't be found
      ...
      browser.execute('eyesEmitter.emit(\'eyes:abortIfNotClosedDone\')');
    }
  });
};
