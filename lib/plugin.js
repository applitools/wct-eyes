...
module.exports = (wct, pluginOptions) => {
  ...
  wct.hook('cleanup', (done) => {
    ...
    Object.values(eyesResults).forEach((testResult) => {
      ...
      const testName = `'${testResult.getName()}' of '${testResult.getAppName()}'`;

      if (![TestResultsStatus.Unresolved, TestResultsStatus.Failed].includes(testResult.getStatus())) {
        console.log(chalk.green(`Test ${testName} passed.`));
      } else if (testResult.getIsNew()) {
        console.log(chalk.yellow(`New test ${testName} ended.`));
      } else {
        console.log(chalk.red(`Test ${testName} detected differences!`));
      }
    });
  ...
  wct.on('eyes:open', (def, data, stats, browser) => {
    const client = new HttpClient(browser.configUrl.href);
