...
const { TestResultsStatus, GeneralUtils, BatchInfo, PerformanceUtils } = require('@applitools/eyes-sdk-core');
...
module.exports = (wct, pluginOptions) => {
  ...
  wct.hook('configure', (done) => {
    PerformanceUtils.start('hook:configure-cleanup');
    PerformanceUtils.start('hook:configure');
    ...
    wct.emit('log:debug', 'WctEyesTime:', p(PerformanceUtils.end('hook:configure')));
    PerformanceUtils.start('wct:after-configure');
    done();
  });

  wct.hook('define:webserver', (app, mapper, options, done) => {
    wct.emit('log:debug', 'WctEyesTime:', p(PerformanceUtils.end('wct:after-configure')));
    PerformanceUtils.start('define:webserver');
    ...
    wct.emit('log:debug', 'WctEyesTime:', p(PerformanceUtils.end('define:webserver')));
    PerformanceUtils.start('wct:after-webserver');
    done();
  });
  ...
  wct.on('eyes:open', async (def, data, stats, browser) => {
    wct.emit('log:debug', 'WctEyesTime:', p(PerformanceUtils.end('wct:after-webserver')));
    PerformanceUtils.start('eyes:open');
    ...
    wct.emit('log:debug', 'WctEyesTime:', p(PerformanceUtils.end('eyes:open')));
    PerformanceUtils.start('wct:after-eyes-open');
  });

  wct.on('eyes:checkWindow', async (def, data, stats, browser) => {
    wct.emit('log:debug', 'WctEyesTime:', p(PerformanceUtils.end('wct:after-eyes-open')));
    PerformanceUtils.start('eyes:checkWindow');
    ...
    wct.emit('log:debug', 'WctEyesTime:', p(PerformanceUtils.end('eyes:checkWindow')));
    PerformanceUtils.start('wct:after-eyes-checkWindow');
  });

  wct.on('eyes:close', async (def, data, stats, browser) => {
    wct.emit('log:debug', 'WctEyesTime:', p(PerformanceUtils.end('wct:after-eyes-checkWindow')));
    PerformanceUtils.start('eyes:close');
    const eyes = eyesSessions[data.sessionId];
    ...
    wct.emit('log:debug', 'WctEyesTime:', p(PerformanceUtils.end('eyes:close')));
    PerformanceUtils.start('wct:after-eyes-close');
  });

  wct.on('eyes:abortIfNotClosed', async (def, data, stats, browser) => {
    wct.emit('log:debug', 'WctEyesTime:', p(PerformanceUtils.end('wct:after-eyes-close')));
    PerformanceUtils.start('eyes:abortIfNotClosed');
    const eyes = eyesSessions[data.sessionId];
    ...
    wct.emit('log:debug', 'WctEyesTime:', p(PerformanceUtils.end('eyes:abortIfNotClosed')));
    PerformanceUtils.start('wct:after-eyes-abortIfNotClosed');
  });

  wct.hook('cleanup', (done) => {
    wct.emit('log:debug', 'WctEyesTime:', p(PerformanceUtils.end('wct:after-eyes-abortIfNotClosed')));
    PerformanceUtils.start('hook:cleanup');
    ...
    wct.emit('log:debug', 'WctEyesTime:', p(PerformanceUtils.end('hook:cleanup')));
    wct.emit('log:debug', 'WctEyesTime:', p(PerformanceUtils.end('hook:configure-cleanup')));
    done();
  });
};
